...
def merge_data_dictionaries(dict1, dict2):
	"""
	Merges two dictionaries of NumPy arrays by concatenating the arrays 
	for common keys. Assumes dict1 is the existing data structure 
	and dict2 is the new data.
	"""
	if dict1 is None:
		return dict2
	
	# Check if dict2 is None or empty, return dict1 if so
	if dict2 is None or not dict2:
		return dict1
	
	merged = {}
	for key in dict1.keys():
		if key in dict2:
			# Concatenate the NumPy arrays for each column
			merged[key] = np.concatenate([dict1[key], dict2[key]])
		else:
			# Keep data from dict1 if key not in dict2 (shouldn't happen often)
			merged[key] = dict1[key]

	# Handle keys only present in dict2 (shouldn't happen with consistent ROOT files)
...
def merge_mass_points(dataset, pkl_outdir):
	...
	if 'HAHM' in dataset_name:
		...
		dataset_output = {}
		mass_points = datasets[dataset_name]
		for mass_entry in mass_points:  ## mass_point = [{M1: ...}, {M2:...}, ...]
			for mass_point, dirs in mass_entry.items():
				...
				for dir_path in dirs:
					...
					merged_data_dict = merge_data_dictionaries(merged_data_dict, result['data'])
		...
		columns_of_interest = kinematics_cols + gen_cols
		...
		for dataset_name, dirs in datasets.items():
			...
			dataset_output = {"data": merged_data_dict, "total_entries": total_events_sum}
			print(f"\tDone: {total_events_sum} total events")
